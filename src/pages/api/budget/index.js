...
import BudgetData from "@/server/Database/models/BudgetData";
  ...
  try {
    switch (method) {
      case 'GET':
        ...
        if (uniqueId) {
          const data = await BudgetData.findOne({
      ...
      case 'POST':
        ...
        if (postUniqueId) {
          ...
          const existingData = await BudgetData.findOne({ where: { uniqueId: postUniqueId, userId } });
          if (existingData) {
            await existingData.update({ data });
            return res.status(200).json({ message: 'Data updated successfully' });
          }
          // Fallback to creation if not found
          const newData = await BudgetData.create({ userId, date: postData, data, uniqueId: postUniqueId });
          return res.status(201).json(newData);
        } else {
          // Create new data entry with uniqueId
          const newUniqueData = await BudgetData.create({
            userId,
            date: postData,
            data,
      ...
      case 'DELETE':
        ...
        const deleted = await BudgetData.destroy({
